...
const BMapURL =
  "https://api.map.baidu.com/api?v=2.0&type=webgl&ak=N6wH5jFd6P3h82VpvutGnO9MwuL6hBqK";
...
const Generating = () => {
  ...
  const [selectedKeys, setSelectedKeys] = useState(["quanguo"]);
    ...
    http
      ...
      .then((res) => {
        if (res.data.code === 200) {
          ...
          const operatingProjects = list.filter(
            (item) => item.nodeState === "运营中"
          );

          // console.log(operatingProjects);
          ...
          // setNodeTypeList(nodeTypes)
          setSelectedKeys(["quanguo"]);
          setSelectCity("全国");
          const groupedAndCounted = groupAndCountNodeTypesdata(cityTree);
  ...
  const extractUniqueNodes = (data) => {
    ...
    const result = Array.from(uniqueNodes).map((jsonStr) =>
      JSON.parse(jsonStr)
    );
  ...
  const onSelect = (selectedKeys, info) => {
    ...
    if (info.node.title === "全国") {
      ...
      if (info.node.type !== "项目") {
        ...
        if (resitem) {
          const operatingProjects = nodesWithType?.filter(
            (item) => item.nodeState === "运营中"
          );
          setEnergylist(operatingProjects);
          setIncomeval(resitem.id);
        ...
        setSelectCity("");
        setCityval("");
        const nodesWithCounts = countNodeTypeOccurrences(
          treeDataList,
          info.node.key
        );
  ...
  const filterBuildingPark = (array) => {
    return array.reduce((acc, item) => {
      if (item.nodeType !== null && item.nodeType !== filtetname) {
        ...
        const newItem = {
          ...item,
          children: filterBuildingPark(item.children),
        };
        acc.push(newItem);
        return acc;
      }
  ...
  const findBuildingsInProvinceOrCity = (
    data,
    targetTitle,
    targetType,
    filtetname
    ...
    if (filtetname) {
      if (filtetname !== "全部") {
        if (selectCity === "全国") {
          ...
          const buildingsInZhejiang =
            cityname &&
            findBuildingsInProvinceOrCity(
              treeDataList,
              cityname.node.title,
              cityname.node.type,
              filtetname
  ...
  const getNodesInfoCount = (val) => {
    http
      .post("homePage/getNodesInfoCount", {
        nodes: val,
      })
      .then((res) => {
        if (res.data.code === 200) {
          ...
          setLoadJieRu(
            data?.loadJieRu !== null &&
              data?.loadJieRu !== undefined &&
              data?.loadJieRu !== ""
              ? Number(data.loadJieRu).toFixed(2)
              : "-"
          );
          setLoadKeTiao(
            data?.loadKeTiao !== null &&
              data?.loadKeTiao !== undefined &&
              data?.loadKeTiao !== ""
              ? Number(data.loadKeTiao).toFixed(2)
              : "-"
          );
    ...
    <div className="generating-cockpit">
      ...
      <div className="generating-cockpit-type">
        <div className="generating-cockpit-province">
          <div
            className={
              isVisible
                ? "generating-cockpit-province-selectval"
                : "generating-cockpit-province-noselectval"
            }
          ...
          <div
            className={`fade-in ${
              isVisible ? "visible" : "generating-cockpit-province-tree"
            }`}
          >
            {isVisible ? (
              <Tree
                ...
                <span>
                  {res.nodeType !== "全部" ? (
                    <i
                      className={
                        res.nodeType === "智算中心"
                          ? "generating-cockpit-province-zhisuan"
                          : res.nodeType === "智能交通"
                          ? "generating-cockpit-province-jiaotong"
                          : res.nodeType === "楼宇园区"
                          ? "generating-cockpit-province-louyu"
                          : res.nodeType === "智能制造"
                          ? "generating-cockpit-province-zhizao"
                          : ""
                      }
                    ></i>
                  ) : null}
        ...
        <div className="generating-cockpit-Energy-income-block-chart">
          <div className="generating-cockpit-Energy-block">
            <div className="generating-cockpit-Energy-block-title">
              ...
              <RangePicker
                onChange={changeDay}
                style={{ width: 246, height: 32 }}
                value={
                  startTs != "" && endTs != ""
                    ? [dayjs(startTs, dateFormat), dayjs(endTs, dateFormat)]
                    : undefined
                }
              />
            </div>
          ...
          <div className="generating-cockpit-Energy-block">
            <div className="generating-cockpit-Energy-block-title">
              ...
              <div className="hand-drawn-line-day">
                <div
                  onClick={() => drawnline(1)}
                  className={drawnlineindex === 1 ? "drawactive" : ""}
                >
                  月
                </div>
                <div
                  onClick={() => drawnline(2)}
                  className={drawnlineindex === 2 ? "drawactive" : ""}
                >
                  年
                </div>
              </div>
      ...
      <div className="generating-cockpit-quantity-of-electricity">
        <div className="generating-cockpit-quantity-of-electricity-load">
          ...
          <div>{<Loadecharts dataList={dataList} infoNode={infoNode} />}</div>
        </div>
        <div className="generating-cockpit-quantity-of-electricity-load">
          <div className="generating-cockpit-quantity-of-electricity-load-title">
            <span className="generating-cockpit-Energy-block-title-header">
