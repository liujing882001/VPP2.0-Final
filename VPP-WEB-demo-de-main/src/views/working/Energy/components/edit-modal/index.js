...
import { DatePicker, TimePicker,Modal,Radio,Input,message, InputNumber } from 'antd';
import http from '../../../../../server/server'
const { RangePicker } = DatePicker;

export const EditModal = (props) => {
    const { editModalVisible,closeEditModal,selected,disabledStartDate,disabledEndDate,strategyList,onQuery,currentNodeId,currentSystem,setTableData,tableData,setStrategyList } = props || {}
        ...
        http.post('system_management/energy_model/energy_storage_model/saveStorageEnergyStrategy',{
            ...
            if(res.data.code !== 200){
                ...
                if(strategyList.length){
                    index = strategyList[strategyList?.length - 1]?.index + 1
                }
    ...
    return (
        ...
        >
                ...
                <p className='edit-modal-item'>日期范围：
                    <RangePicker 
                        format="YYYY-MM-DD" 
                        defaultValue={[startDate && dayjs(startDate, 'YYYY-MM-DD'),endDate && dayjs(endDate, 'YYYY-MM-DD')]} 
