...
class ModelMainClass(BaseModelMainClass):
    ...
    def process_data(self, input_data, now_time, start_time):
        ...
        # 工作日数据
        # ---------------
        df_load_workday = copy.deepcopy(df_load.query("(date_type == 1)"))  # 工作日
        ...
        # 非节日的周六数据
        # ---------------
        df_load_Saturday = copy.deepcopy(
            df_load.query("((date_type == 2) and (datetime_weekday == 5))")
        )  # 非节日的周六
        data_X_Saturday = df_load_Saturday[feature_list]
        data_Y_Saturday = df_load_Saturday["load"]
        logger.info(
            f"project: {self.project},  model: {self.model},  node: {self.node}:: length of df_load_workday {len(df_load_Saturday)}"
        )
        # 非工作日数据（节日和非节日的周日）
        # ---------------
    ...
    def train(
        ...
        # 计算模型的性能指标
        logger.info(
            f"project: {self.project},  model: {self.model},  node: {self.node},  type: {type}:: r2:{r2_score(Y_test,  Y_predicted)}"
        )
        logger.info(
            f"project: {self.project},  model: {self.model},  node: {self.node},  type: {type}:: mse:{mean_squared_error(Y_test,  Y_predicted)}"
        )
        logger.info(
            f"project: {self.project},  model: {self.model},  node: {self.node},  type: {type}:: mae:{mean_absolute_error(Y_test,  Y_predicted)}"
        )
        accuracy = 1 - mean_absolute_percentage_error(Y_test, Y_predicted)
        logger.info(
            f"project: {self.project},  model: {self.model},  node: {self.node},  type: {type}:: mape accuracy:{accuracy}"
        )
    ...
    def predict(
        ...
        lgb_model_Saturday: lgb.LGBMRegressor,
        ...
        # 工作日模型预测
        df_future_workday = copy.deepcopy(df_future.query("(date_type == 1)"))  # 工作日
        ...
        # 非节日的周六模型预测
        df_future_Saturday = copy.deepcopy(
            df_future.query("((date_type == 2) and (datetime_weekday == 5))")
        )  # 非节日的周六
        X_future_Saturday = df_future_Saturday[feature_list]
        Y_future_Saturday = lgb_model_Saturday.predict(X_future_Saturday)
        df_future_Saturday["load"] = Y_future_Saturday
        logger.info(
            f"project: {self.project},  model: {self.model},  node: {self.node}:: df_future_workday is \n {df_future_Saturday.iloc[-10:]}"
        )
        # 非工作日模型预测
        ...
        df_future = pd.merge(df_future, df_future_Saturday, how="outer")
    ...
    def run(self, input_data: Dict, model_cfgs: Dict, output_func: Dict = None):
        ...
        lgb_model_Saturday = self.train(
            data_X_Saturday,
            data_Y_Saturday,
            model_cfgs["lgbm_params_Saturday"],
            data_length=96 * 15,
            split_length=96,
            type="Saturday",
        )
...
def main():
    ...
    now = (
        datetime.datetime(2024, 11, 13, 16, 0, 0)
    ...
    model_cfgs = {
        "nodes": {
            "node": {
                "node_id": "f79237fc44855884911d8136ba431f5c",
    ...
    res = model_cls.run(model_cfgs=model_cfgs)
