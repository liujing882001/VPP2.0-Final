...
class ModelMainClass(BaseModelMainClass):
    ...
    def calculate(self, df: pd.DataFrame, model_cfgs: Dict = {}):
        ...
        if (date_type == 1) or ((date_type == 2) and (weekday == 5)):
            df.loc[80:81, "power_final"] = -215
            df.loc[82:83, "power_final"] = 0
        # 节日和非节日的周日的平时段不充电
        # weekday  =  int(df["timeStamp"].apply(lambda x: x.weekday()).unique()[0])
        if (date_type > 2) or ((date_type == 2) and (weekday == 6)):
            df.loc[df["property"] == "平", "power_final"] = 0
        logger.info(
