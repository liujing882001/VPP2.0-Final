...
interface PowerGenDeviceAPI {
  ...
  // 更新设备信息
  "PUT /api/v2/powergen/devices/{deviceId}": {
    params: {
      deviceId: string;
    };
  ...
  // 删除设备
  "DELETE /api/v2/powergen/devices/{deviceId}": {
    params: {
      deviceId: string;
    };
...
interface PowerForecastAPI {
  ...
  // 预测模型管理
  "GET /api/v2/powergen/forecast/models": {
    response: APIResponse<ForecastModel[]>;
  };
  
  // 更新预测模型
  "PUT /api/v2/powergen/forecast/models/{modelId}": {
    params: {
      modelId: string;
    };
    request: {
    ...
    response: APIResponse<ModelUpdateResult>;
  };
}
...
interface StorageOptimizationAPI {
  ...
  // 获取优化结果
  "GET /api/v2/powergen/storage/{systemId}/optimization": {
    params: {
      systemId: string;
    };
    query: {
  ...
  // 执行储能控制指令
  "POST /api/v2/powergen/storage/{systemId}/control": {
    params: {
      systemId: string;
    };
    request: {
    ...
    response: APIResponse<ControlResponse>;
  };
}
...
interface SmartLoadMonitoringAPI {
  ...
  // 获取负荷统计分析
  "GET /api/v2/smartload/load/analytics": {
    query: {
      meterId: string;
      period: "24hour" | "7day" | "30day" | "1year";
      metrics: string[]; // ["peak", "average", "loadFactor", "demandPattern"]
    };
    response: APIResponse<LoadAnalytics>;
  };
}
...
interface LoadOptimizationAPI {
  ...
  // 执行优化策略
  "POST /api/v2/smartload/optimization/{jobId}/execute": {
    params: {
      jobId: string;
    };
    request: {
      selectedActions: string[];
      executionMode: "automatic" | "manual_confirm";
    };
...
interface DemandResponseAPI {
  ...
  // 响应DR事件
  "POST /api/v2/smartload/demand-response/events/{eventId}/respond": {
    params: {
      eventId: string;
    };
    request: {
      participationDecision: "accept" | "decline" | "partial";
      committedReduction?: number; // kW
      reason?: string;
    };
    response: APIResponse<ResponseConfirmation>;
  };
  
  // 获取DR事件历史
  "GET /api/v2/smartload/demand-response/events/history": {
    query: {
      meterId: string;
      startDate: string;
      endDate: string;
    ...
    response: APIResponse<DREventHistory>;
  };
}
...
interface MarketTradingAPI {
  ...
  // 实时市场交易
  "POST /api/v2/vppcloud/markets/real-time/trade": {
    request: {
      vppId: string;
      tradeType: "buy" | "sell";
      quantity: number; // MW
    ...
    response: APIResponse<TradeExecutionResult>;
  };
  
  // 辅助服务投标
  "POST /api/v2/vppcloud/markets/ancillary/bids": {
    request: {
      vppId: string;
      serviceType: "frequency_regulation" | "spinning_reserve" | "voltage_support";
      capacity: number; // MW
      price: number; // 元/MW
      availabilityPeriod: {
        startTime: string;
    ...
    response: APIResponse<AncillaryBidResult>;
  };
}
...
```typescript
// 实时调度API
interface RealTimeDispatchAPI {
  // 生成调度指令
  "POST /api/v2/vppcloud/dispatch/instructions": {
    request: {
      vppId: string;
      dispatchPeriod: {
        startTime: string;
      ...
      targetProfile: number[]; // MW setpoints
      constraints: {
        rampLimits: RampConstraints;
        operatingLimits: OperatingLimits;
        priorityResources: string[];
      };
      optimizationObjective: "cost_minimize" | "response_maximize" | "stability_optimize";
    };
    response: APIResponse<DispatchInstructions>;
  };
  
  // 执行调度指令
  "POST /api/v2/vppcloud/dispatch/execute": {
    request: {
      instructionId: string;
      executionMode: "immediate" | "scheduled";
      confirmationRequired: boolean;
    };
    response: APIResponse<DispatchExecution>;
  };
  
  // 监控调度执行状态
  "GET /api/v2/vppcloud/dispatch/{instructionId}/status": {
    params: {
      instructionId: string;
    };
    response: APIResponse<DispatchStatus>;
  };
  
  // 获取系统状态估计
  "GET /api/v2/vppcloud/system/state-estimation": {
    query: {
      vppId: string;
      timestamp?: string;
    };
    response: APIResponse<SystemStateEstimation>;
  };
}
...
interface DispatchInstructions {
  ...
  instructions: {
    ...
    setpoints: {
      ...
      activePower: number; // MW
      reactivePower: number; // MVAr
      mode: "generation" | "consumption" | "standby";
    }[];
...
interface UserManagementAPI {
  ...
  "POST /api/v2/users/register": {
    ...
    response: APIResponse<UserRegistration>;
  };
...
interface MonitoringAPI {
  ...
  "GET /api/v2/system/metrics": {
    query: {
      metricType: "api_performance" | "resource_usage" | "business_kpi";
      timeRange: string;
  ...
  "POST /api/v2/reports/generate": {
    request: {
      reportType: "energy_summary" | "financial_analysis" | "performance_report";
      parameters: {
        startDate: string;
        endDate: string;
        entities: string[]; // device IDs, meter IDs, or VPP IDs
...
interface SystemHealth {
  ...
  components: {
    api_gateway: ComponentHealth;
    databases: ComponentHealth;
    ai_services: ComponentHealth;
    external_integrations: ComponentHealth;
  };
  metrics: {
    uptime: number; // %
...
async function managePowerGeneration() {
  ...
  const optimization = await fetch('/api/v2/powergen/storage/optimize', {
    ...
    body: JSON.stringify({
      ...
      demandForecast: [100, 120, 140, ...], // 96个点，15分钟间隔
      priceSchedule: {
        peak: [0.8, 0.9, 1.0],
        flat: [0.5, 0.6, 0.7], 
        valley: [0.3, 0.4, 0.5]
